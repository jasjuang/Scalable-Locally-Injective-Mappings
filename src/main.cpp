...
void check_mesh_for_issues(Eigen::MatrixXd& V,
                           Eigen::MatrixXi& F,
...
#ifndef USE_PARDISO
  cout << "Warning! Unoptimized version without Pardiso Solver. The algorithm "
...
void read_mesh(const std::string& mesh_file, Param_State& state)
{
  ...
  // set uv coords scale
  igl::doublearea(state.V, state.F, state.M);
  state.M /= 2.;
                           ...
                           Eigen::VectorXd& areas)
{
  ...
  if (!euler_char)
  {
    cout << "Error! Input does not have a disk topology, it's euler char is "
         << euler_char << endl;
    exit(1);
  }
