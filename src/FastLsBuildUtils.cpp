...
class FastATA_Multiplier
{
 ...
 public:
  void operator()(const tbb::blocked_range<size_t>& r) const
  {
    ...
    {
      const int cur_idx  = inst_idx[i];
      const int next_idx = inst_idx[i + 1];
      double val         = 0;
      for (int j = 0; j < next_idx - cur_idx; j++)
      {
        val += k1[instructions[cur_idx + j].src1] *
               k2[instructions[cur_idx + j].src2];
      }
      K[instructions[cur_idx].dst] += val;
    }
  }
  FastATA_Multiplier(const Eigen::VectorXd& k1,
                     const Eigen::VectorXd& k2,
                     Eigen::VectorXd& K,
                     const instruction_list& instructions,
                     const std::vector<int>& inst_idx)
...
void add_dx_mult_dx_to_K(const Eigen::VectorXd& k1,
                         const Eigen::VectorXd& k2,
                         Eigen::VectorXd& K,
                         const instruction_list& instructions,
                         const std::vector<int>& inst_idx)
{
  ...
  parallel_for(tbb::blocked_range<size_t>(0, values_n),
               FastATA_Multiplier(k1, k2, K, instructions, inst_idx),
               ap);
}

void build_dx_k_maps(const int v_n,
                     const Eigen::MatrixXi& F,
                     Eigen::VectorXi& ai,
                     Eigen::VectorXi& aj,
                     instruction_list& instructions1,
                     instruction_list& instructions2,
                     instruction_list& instructions4,
                     std::vector<int>& inst1_idx,
                     std::vector<int>& inst2_idx,
                     std::vector<int>& inst4_idx)
{
  using namespace std;
  ...
  std::cout << "Started: " << timer.getElapsedTime() << endl;
  vector<vector<int> > V2V_tmp;
  igl::adjacency_list(F, V2V_tmp, false /*sorted*/);
  std::cout << "Built adjacency list: " << timer.getElapsedTime() << endl;
  // extending V2V to 2Vx2V, adj(2Vx2V) = [adj[V],adj[V]+v_n;
  // adj[V],adj[V]+v_n];
  ...
  std::cout << "Built V2V: " << timer.getElapsedTime() << endl;
  // build vv_to_K (but we now look at K as a 2Vx2V sparse matrix)
  std::vector<int> ai_v, aj_v;
  ai_v.reserve(2 * v_n);
  aj_v.reserve(12 * v_n);
  ai_v.push_back(1);
  int K_idx = 0;
  ...
  std::cout << "Built vv_to_K: " << timer.getElapsedTime() << endl;

  int ai_size = ai_v.size();
  int aj_size = aj_v.size();
  ...
  vector<vector<int> > V2F;
  vector<vector<int> > V2Fi;
  igl::vertex_triangle_adjacency(v_n, F, V2F, V2Fi);
  std::cout << "Built V2F: " << timer.getElapsedTime() << endl;

  instructions1.reserve(3 * 3 * f_n);
  instructions2.reserve(3 * 3 * f_n);
  instructions4.reserve(3 * 3 * f_n);
  for (int vi = 0; vi < v_n; vi++)
  {
    ...
    for (int f_i = 0; f_i < f_nbd; f_i++)
    {
      ...
      for (int j = 0; j < 3; j++)
      {
        ...
        // now we have both of the indexes k1,k2 we need to multiply, but we
        // have 3 different possible K index destination
        //	this is based on K1,K2,K4 indexes, so we add 3 instructions to
        // the 3 lists (where K_mat = [K1,K2;K3,K4] is a symmetric matrix)
        ...
        if (vi <= vjf)
        {
          ...
          // cout << "vjf = " << vjf << " fvj_k = " << fvj_k << " inst1_dst_K =
          // " << inst1_dst_K << endl;
  ...
  std::cout << "Built basic instructions: " << timer.getElapsedTime() << endl;
  ...
  std::cout << "Sorted instructions: " << timer.getElapsedTime() << endl;

  // build the reference index
  inst2_idx.reserve(aj_size /
                    2);  // should be of size (2K)/4 = 0.5*K (since all is in)
  inst1_idx.reserve(aj_size / 3);
  inst4_idx.reserve(aj_size / 3);  // should be of size (0.25*K -0.5*diag_size)
  ...
  std::cout << "finished building instructions: " << timer.getElapsedTime()
            << endl;
}

void build_dx_k_maps_3d(const int v_n,
                        const Eigen::MatrixXi& F,
                        Eigen::VectorXi& ai,
                        Eigen::VectorXi& aj,
                        instruction_list& instructions1,
                        instruction_list& instructions2,
                        instruction_list& instructions3,
                        instruction_list& instructions5,
                        instruction_list& instructions6,
                        instruction_list& instructions9,
                        std::vector<int>& inst1_idx,
                        std::vector<int>& inst2_idx,
                        std::vector<int>& inst3_idx,
                        std::vector<int>& inst5_idx,
                        std::vector<int>& inst6_idx,
                        std::vector<int>& inst9_idx)
{
  using namespace std;
  using namespace Eigen;
  ...
  std::cout << "Built adjacency list: " << timer.getElapsedTime() << endl;
  // extending V2V to 3Vx3V, adj(3Vx3V) = [adj[V],adj[V]+v_n,adj[V]+2*v_n;
  // adj[V],adj[V]+v_n.adj[V]+2*v_n] (similar to the 2d function)
  ...
  instructions1.reserve(6 * 4 * f_n);
  instructions2.reserve(6 * 4 * f_n);
  instructions3.reserve(6 * 4 * f_n);
  instructions5.reserve(6 * 4 * f_n);
  instructions6.reserve(6 * 4 * f_n);
  instructions9.reserve(6 * 4 * f_n);
  for (int vi = 0; vi < v_n; vi++)
  {
    int f_nbd = V2F[vi].size();
    for (int f_i = 0; f_i < f_nbd; f_i++)
    {
      ...
      for (int j = 0; j < 4; j++)
      {
        ...
        // now we have both of the indexes k1,k2 we need to multiply, but we
        // have 6 different possible K index destination
        //	this is based on K1,K2,K3,K5,K6,K9 indexes, so we add 3
        // instructions to the 3 lists (where K_mat =
        //[K1,K2,K3;K4,K5,K6;K7,K8,K9] is symmetric)
        int inst2_dst_K = vv_to_K[vv_p(vi, v_n + vjf)];
        instructions2.push_back({fvi_k, fvj_k, inst2_dst_K});
  ...
  // build the reference index
  int full_num = ceil(2 * aj_size / 9.);
  int part_num = ceil(aj_size / 9. + v_n);
  inst2_idx.reserve(full_num);
  inst3_idx.reserve(full_num);
  inst6_idx.reserve(full_num);
  inst1_idx.reserve(part_num);
  inst5_idx.reserve(part_num);
  inst9_idx.reserve(part_num);
...
void build_index_from_instructions(instruction_list& instructions,
                                   std::vector<int>& inst_idx)
{
  inst_idx.push_back(0);
  int cur_dst = instructions[0].dst;
  int i       = 0;
...
void multiply_dx_by_W(const Eigen::VectorXd& k1,
                      const Eigen::VectorXd& face_W,
                      Eigen::VectorXd& res)
{
  if (res.rows() < k1.rows())
  {
...
void multiply_dx_by_W_3d(const Eigen::VectorXd& k1,
                         const Eigen::VectorXd& face_W,
                         Eigen::VectorXd& res)
{
  if (res.rows() < k1.rows())
...
void dx_to_csr(const Eigen::SparseMatrix<double>& L,
               Eigen::VectorXi& ia,
               Eigen::VectorXi& ja,
               Eigen::VectorXd& a)
{
  ...
  // todo: make sure diagonal terms are included, even as zeros (pardiso claims
  // this is necessary for best performance)
  ...
  II.reserve(expected_size);
  JJ.reserve(expected_size);
  SS.reserve(expected_size);
  ...
  for (int i = 0; i < II.size(); ++i) M0.row(i) << II[i], JJ[i], i;
  ...
  while (si < M_.rows())
  {
    currI = M_(si, 0);
    ei    = si;
    while (ei < M_.rows() && M_(ei, 0) == currI) ++ei;
  ...
  ia.setZero(numRows + 1, 1);
  ia(numRows) = numUniqueElements + 1;
  ja          = M.col(1).array() + 1;
  currI       = -1;
  for (int i = 0; i < numUniqueElements; ++i)
  {
...
void csr_to_mat(int m,
                int n,
                const Eigen::VectorXi& ai,
                const Eigen::VectorXi& aj,
                ...
                Eigen::SparseMatrix<double>& A)
{
  ...
  for (int i = 0; i < m; i++)
  {
    ...
    {
      IJV.push_back(Eigen::Triplet<double>(
          i, aj[k_idx] - 1, a[k_idx]));  // the j are 1-based!
      k_idx++;
    }
  }
...
void tet_adjacency_list(const Eigen::MatrixXi& F,
