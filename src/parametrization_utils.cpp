...
void compute_surface_gradient_matrix(const Eigen::MatrixXd& V,
                                     ...
                                     Eigen::SparseMatrix<double>& D2)
{
  ...
  for (int i = 0; i < fn; i++)
  {
    ...
    // grad3_3f(:,[3*i,3*i-2,3*i-1])=[0,-Fni(3),
    // Fni(2);Fni(3),0,-Fni(1);-Fni(2),Fni(1),0]*e/(2*Ari);
  ...
  SparseMatrix<double> Dx(fn, vn);
  Dx.setFromTriplets(Gx_trip.begin(), Gx_trip.end());
  SparseMatrix<double> Dy(fn, vn);
  Dy.setFromTriplets(Gy_trip.begin(), Gy_trip.end());
  SparseMatrix<double> Dz(fn, vn);
  Dz.setFromTriplets(Gz_trip.begin(), Gz_trip.end());

  // This is good only if G = [Dx;Dy;Dz]. If not, then we should build the
  // matrices in a different way (I guess a for loop?)
  /*
  ...
  */
  ...
  Eigen::VectorXi Rx = 3*R; Eigen::VectorXi Ry = 3*R.array() +1; Eigen::VectorXi
  Rz = 3*R.array() +2; Eigen::VectorXi C = igl::colon<int>(0,G.cols()-1);
                                     ...
                                     bool flips_linesearch)
{
  ...
  for (int i = 0; i < f_n; i++)
  {
    ...
    if (flips_linesearch)
    {
      ...
      // conf_e += areas(i) * (pow(log(s1/sigma_geo_avg),2) +
      // pow(log(s2/sigma_geo_avg),2));
      conf_e += areas(i) * ((pow(s1, 2) + pow(s2, 2)) / (2 * s1 * s2));
      norm_arap_e += areas(i) * (pow(s1 - 1, 2) + pow(s2 - 1, 2));
      amips +=
          areas(i) * exp(exp_factor * (0.5 * ((s1 / s2) + (s2 / s1)) +
                                       0.25 * ((s1 * s2) + (1. / (s1 * s2)))));
      exp_symmd += areas(i) * exp(exp_factor * (pow(s1, 2) + pow(s1, -2) +
                                                pow(s2, 2) + pow(s2, -2)));
      // amips +=  areas(i) * exp(  0.5*( (s1/s2) +(s2/s1) ) + 0.25*( (s1*s2) +
      // (1./(s1*s2)) )  ) ;
    }
    else
    {
      ...
      {
        // it is the distance form the flipped thing, this is slow, usefull only
        // for debugging normal arap
        vi.col(1) *= -1;
...
void map_vertices_to_circle_area_normalized(const Eigen::MatrixXd& V,
                                            const Eigen::MatrixXi& F,
                                            ...
                                            Eigen::MatrixXd& UV)
{
  ...
  cout << "map_vertices_to_circle_area_normalized, area = " << area
       << " radius = " << radius << endl;
               ...
               std::vector<int>& flip_idx)
{
  ...
  for (int i = 0; i < F.rows(); i++)
  {
    ...
    if (det < 0)
    {
      // cout << "flip at face #" << i << " det = " << T2_Homo.determinant() <<
      // endl;
                         ...
                         Eigen::MatrixXd& uv)
{
  ...
  SparseMatrix<double> Quu;
  igl::slice(Q, unknown, unknown, Quu);
  SparseMatrix<double> Qub;
  igl::slice(Q, unknown, b, Qub);
  ...
  for (int k = 0; k < Quu.outerSize(); ++k)
  {
    for (Eigen::SparseMatrix<double>::InnerIterator it(Quu, k); it; ++it)
    {
      if (it.row() <= it.col())
      {
        ii.push_back(it.row());
        jj.push_back(it.col());
        kk.push_back(it.value());
      }
    }
  }

  Eigen::VectorXi ai, ji;
  Eigen::VectorXd a;
  ...
  for (int w = 0; w < bc.cols(); w++)
  {
    ...
    // cout << "known_idx = " << known_idx << " unknown_idx = " << unknown_idx
    // << endl;
  }
#endif
}
...
int get_euler_char(const Eigen::MatrixXd& V, const Eigen::MatrixXi& F)
{
  int euler_v = V.rows();
