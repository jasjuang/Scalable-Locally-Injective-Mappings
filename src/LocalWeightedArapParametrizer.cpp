...
#ifdef USE_PARDISO
LocalWeightedArapParametrizer::LocalWeightedArapParametrizer(Param_State* state,
                                                             bool remeshing)
...
void LocalWeightedArapParametrizer::parametrize(const Eigen::MatrixXd& V,
...
void LocalWeightedArapParametrizer::compute_jacobians(const Eigen::MatrixXd& uv)
{
  ...
  for (int i = 0; i < f_n; i++)
  {
    const int col1 = dxj[k_idx] - 1;
    const int col2 = dxj[k_idx + 1] - 1;
    const int col3 = dxj[k_idx + 2] - 1;

    const double a_x_k   = a_x(k_idx);
    const double a_x_k_1 = a_x(k_idx + 1);
    const double a_x_k_2 = a_x(k_idx + 2);

    Ji(i, 0) = a_x_k * uv(col1, 0) + a_x_k_1 * uv(col2, 0) +
               a_x_k_2 * uv(col3, 0);  // Dx*u
    Ji(i, 2) = a_x_k * uv(col1, 1) + a_x_k_1 * uv(col2, 1) +
               a_x_k_2 * uv(col3, 1);  // Dx*v

    const double a_y_k   = a_y(k_idx);
    const double a_y_k_1 = a_y(k_idx + 1);
    const double a_y_k_2 = a_y(k_idx + 2);
    Ji(i, 1)             = a_y_k * uv(col1, 0) + a_y_k_1 * uv(col2, 0) +
               a_y_k_2 * uv(col3, 0);  // Dy*u
    Ji(i, 3) = a_y_k * uv(col1, 1) + a_y_k_1 * uv(col2, 1) +
               a_y_k_2 * uv(col3, 1);  // Dy*v
...
class WeightUpdater
{
 ...
 public:
  ...
  WeightUpdater(Param_State* m_state,
                const Eigen::MatrixXd& Ji,
                Eigen::MatrixXd& Ri,
                Eigen::VectorXd& W_11,
                Eigen::VectorXd& W_12,
...
void LocalWeightedArapParametrizer::update_weights_and_closest_rotations(
    const Eigen::MatrixXd& V, const Eigen::MatrixXi& F, Eigen::MatrixXd& uv)
{
  // cout << "updating weights: " << m_state->timer.getElapsedTime() << endl;
  compute_jacobians(uv);

  static tbb::affinity_partitioner ap;
  parallel_for(tbb::blocked_range<size_t>(0, f_n),
               WeightUpdater(m_state, Ji, Ri, W_11, W_12, W_21, W_22),
               ap);
  // cout << "updated weights: " << m_state->timer.getElapsedTime() << endl;
}

void LocalWeightedArapParametrizer::solve_weighted_arap(
    ...
    Eigen::MatrixXd& soft_bc_p)
{
...
#else
  cout << "********Warning: The version is not optimized. Solving without "
          "Pardiso!************"
       << endl;
  // move to zero based
  ...
  SparseMatrix<double, RowMajor> mat =
      MappedSparseMatrix<double, Eigen::RowMajor>(
          2 * v_n, 2 * v_n, nnz, ai.data(), aj.data(), K.data());
  ...
  igl::min_quad_with_fixed_solve(
      solver_data, negRhs, Eigen::VectorXd(), Beq, Uc);
...
void LocalWeightedArapParametrizer::pre_calc()
{
  if (!has_pre_calc)
  {
...
#ifdef USE_PARDISO
    m_solver.set_type(
        2);  // 2 is PSD, -2 is symmetric indefinite Real symmetric matrix
#endif
    first_solve = true;
    ...
    dx_to_csr(Dx, dxi, dxj, a_x);
    dx_to_csr(Dy, ai_t, aj_t, a_y);

    int ax_size = a_x.rows();
    w11Dx.resize(ax_size);
    w12Dx.resize(ax_size);
    w11Dy.resize(ax_size);
    w12Dy.resize(ax_size);
    w21Dx.resize(ax_size);
    w22Dx.resize(ax_size);
    w21Dy.resize(ax_size);
    w22Dy.resize(ax_size);
...
void LocalWeightedArapParametrizer::get_At_AtMA_fast()
{
  ...
  for (int i = 0; i < f_n; i++)
  {
    ...
    for (int j = 0; j < nnz_in_line; j++)
    {
      ...
      rhs(dest_k) += a_x(A_idx) * W_11(i) * f1_i + a_y(A_idx) * W_11(i) * f2_i +
                     a_x(A_idx) * W_21(i) * f3_i + a_y(A_idx) * W_21(i) * f4_i;
      rhs(dest_k + v_n) +=
          a_x(A_idx) * W_21(i) * f1_i + a_y(A_idx) * W_21(i) * f2_i +
          a_x(A_idx) * W_22(i) * f3_i + a_y(A_idx) * W_22(i) * f4_i;
  ...
  multiply_dx_by_W(a_x, W_11, w11Dx);
  multiply_dx_by_W(a_x, W_12, w12Dx);
  multiply_dx_by_W(a_y, W_11, w11Dy);
  multiply_dx_by_W(a_y, W_12, w12Dy);
  multiply_dx_by_W(a_x, W_21, w21Dx);
  multiply_dx_by_W(a_x, W_22, w22Dx);
  multiply_dx_by_W(a_y, W_21, w21Dy);
  multiply_dx_by_W(a_y, W_22, w22Dy);
  ...
  // K1
  add_dx_mult_dx_to_K(w11Dx, w11Dx, K, inst1, inst1_idx);
  add_dx_mult_dx_to_K(w11Dy, w11Dy, K, inst1, inst1_idx);
  add_dx_mult_dx_to_K(w21Dx, w21Dx, K, inst1, inst1_idx);
  add_dx_mult_dx_to_K(w21Dy, w21Dy, K, inst1, inst1_idx);

  // K2
  add_dx_mult_dx_to_K(w11Dx, w12Dx, K, inst2, inst2_idx);
  add_dx_mult_dx_to_K(w11Dy, w12Dy, K, inst2, inst2_idx);
  add_dx_mult_dx_to_K(w21Dx, w22Dx, K, inst2, inst2_idx);
  add_dx_mult_dx_to_K(w21Dy, w22Dy, K, inst2, inst2_idx);

  // K4
  add_dx_mult_dx_to_K(w12Dx, w12Dx, K, inst4, inst4_idx);
  add_dx_mult_dx_to_K(w12Dy, w12Dy, K, inst4, inst4_idx);
  add_dx_mult_dx_to_K(w22Dx, w22Dx, K, inst4, inst4_idx);
  add_dx_mult_dx_to_K(w22Dy, w22Dy, K, inst4, inst4_idx);

  // cout << "Computed new K: " << m_state->timer.getElapsedTime() << endl;

  // cout << "Added proximal penalty to K: " << m_state->timer.getElapsedTime()
  // << endl;
...
void LocalWeightedArapParametrizer::compute_grad(const Eigen::MatrixXd& V,
                                                 const Eigen::MatrixXi& F,
                                                 Eigen::MatrixXd& uv,
                                                 Eigen::MatrixXd& grad)
{
                                                     ...
                                                     Eigen::MatrixXd& uv)
{
  ...
  else
  {
    ...
    compute_jacobians(uv);
    compute_energies_with_jacobians(V,
                                    F,
                                    Ji,
                                    uv,
                                    m_state->M,
                                    ...
                                    exp_symmd,
                                    m_state->exp_factor,
                                    m_state->flips_linesearch);

    if (m_state->global_local_energy == Param_State::SYMMETRIC_DIRICHLET)
    {
      // cout << "returning symmd energy, time = " <<
      // m_state->timer.getElapsedTime() << endl;
      return symmd_e;
    }
